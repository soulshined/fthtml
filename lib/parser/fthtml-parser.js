...
class ftHTMLParser {
    ...
    parseWhileType(types, endingtype, onendingtype, and_only_n_times = Number.POSITIVE_INFINITY) {
        ...
        while (!this.input.eof() && iterations++ < and_only_n_times) {
            ...
            if (this.isExpectedType(t, "Word") && (this.tinytemplates[t.value] !== undefined || user_config_1.default.tinytemplates[t.value] !== undefined))
                html += this.parseTinyTemplate();
            else if (t.type == "Word")
                html += this.parseTag();
    ...
    parseMaybePragma() {
        ...
        if (pragma.value === 'vars') {
            do {
                ...
                if (peek.type == "String")
                    ...
                    if (json_file.value.startsWith('https:') || json_file.value.startsWith('http:'))
                        throw new index_1.ftHTMLImportError(`Files must be local, can not access '${json_file.value}'`, json_file);
            ...
            throw new index_1.ftHTMLIncompleteElementError(pragma, `an '#end' pragma keyword for starting pragma '${pragma.value}' but none found`, pragma);
        }
        else if (pragma.value.endsWith('templates')) {
            do {
                const tinytempl = this.consume();
                if (this.isExpectedType(tinytempl, 'Pragma_end'))
                    return;
                if (tinytempl.type !== "Word")
                    throw new index_1.ftHTMLInvalidTinyTemplateNameError(tinytempl, "[\w-]+", this.vars._$.__filename);
                if (this.isEOF())
                    throw new index_1.ftHTMLIncompleteElementError(tinytempl, 'a string or single ftHTML element');
                const element = this.consume();
                if (!this.isOneOfExpectedTypes(element, ["Word", "String"]))
                    throw new index_1.ftHTMLInvalidTypeError(element, 'a string or single ftHTML element');
                if (this.isExpectedType(element, "String")) {
                    this.tinytemplates[tinytempl.value] = models_1.TinyTemplate(element, this.vars._$.__filename);
                    continue;
                }
                if (this.isEOF())
                    throw new index_1.ftHTMLIncompleteElementError(pragma, `an '#end' pragma keyword for starting pragma '${pragma.value}' but none found`);
                const peek = this.peek();
                if (this.isExpectedType(peek, 'Pragma_end')) {
                    this.tinytemplates[tinytempl.value] = models_1.TinyTemplate({
                        type: "String",
                        value: '${val}',
                ...
                if (this.isExpectedType(peek, "String")) {
                    this.tinytemplates[tinytempl.value] = models_1.TinyTemplate(this.consume(), this.vars._$.__filename, element);
                }
                else if (this.isExpectedType(peek, 'Symbol_(')) {
                    const parenth = this.consume();
                    if (this.isEOF())
                        throw new index_1.ftHTMLIncompleteElementError(element, 'closing and opening parenthesis');
                    const attrs = this.parseAttributes(parenth);
                    let value;
                    if (this.isExpectedType(this.peek(), "String"))
                        value = this.consume();
                    else if (!this.isExpectedType(this.peek(), "Word") && !this.isExpectedType(this.peek(), 'Pragma_end'))
                        throw new index_1.ftHTMLInvalidTypeError(element, 'a string or single ftHTML element');
                    this.tinytemplates[tinytempl.value] = models_1.TinyTemplate((value !== null && value !== void 0 ? value : {
                        type: "String",
                        value: '${val}',
                        position: element.position
                    }), this.vars._$.__filename, element, attrs);
                }
                else if (this.isExpectedType(peek, "Word"))
                    this.tinytemplates[tinytempl.value] = models_1.TinyTemplate(element, this.vars._$.__filename);
                else
                    throw new index_1.ftHTMLInvalidTypeError(element, 'a string or single ftHTML element');
            } while (!this.isEOF());
    ...
    parseTinyTemplate() {
        ...
        const uconfigtt = user_config_1.default.tinytemplates[word.value];
        const tt = this.tinytemplates[word.value] || uconfigtt;
        let { element, attrs, value } = tt;
        if (uconfigtt !== undefined) {
            try {
                const tts = this.compileTinyTemplate(`#templates ${word.value} ${value.value} #end`);
                value = tts[word.value].value;
                element = tts[word.value].element;
                attrs = tts[word.value].attrs;
            }
            catch (error) { }
        }
        ...
        if (placeholders.length === 0)
            throw new index_1.ftHTMLInvalidTinyTemplatePlaceholderError(word, this.vars._$.__filename);
        let replacement = '';
        if (this.isExpectedType(this.peek(), 'Symbol_{'))
            replacement = this.parseBindingPropertyValueAsFTHTML();
        else if (this.isOneOfExpectedTypes(this.peek(), ["String", "Macro", "Function", "Variable"])) {
            replacement = this.parseWhileType(["String", "Macro", "Function", "Variable"], null, null, 1);
        }
        placeholders.forEach(() => result = result.replace(/(?<!\\)(\${[ ]*val[ ]*})/g, replacement));
        result = this.parseString({ type: "String", value: result, position: word.position });
        return element
            ? this.createElement(this.evaluateENForToken(element), attrs, result)
            : result;
    }
    ...
    initElementWithAttrs(tag) {
        ...
        const attrs = this.parseAttributes(t);
        if (this.isEOF() || self_closing_tags_1.SELF_CLOSING_TAGS.includes(tag))
