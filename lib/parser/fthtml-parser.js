...
class ftHTMLParser {
        ...
        if (pragma.value === 'vars') {
            do {
                ...
                if (peek.type == "String")
                    ...
                    throw new index_1.ftHTMLInvalidTypeError(peek, "String");
    ...
    parseTemplate() {
        ...
        do {
            ...
            if (t.type != "Word" && !this.isExpectedType(t, 'Keyword_import'))
                throw new index_1.ftHTMLInvalidVariableNameError(t, '[\w-]+');
            const peek = this.input.peek();
            if (this.input.eof())
        ...
        do {
            ...
            if (t.type == "Attr_Class")
                ...
                if (this.isExpectedType(peek, 'Symbol_=')) {
                    this.consume();
                    peek = this.peek();
                    if (!["String", "Word", "Variable"].includes(peek.type))
    ...
    initElementWithChildren(tag, attrs) {
        ...
        return this.parseWhileType(["Word", "ELang", "Pragma", "String", "Keyword", "Variable"], 'Symbol_}', (html, error) => {
            if (error)
