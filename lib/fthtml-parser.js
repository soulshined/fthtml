(function () {
  ...
  ftHTMLParser.Parser = (class Parser {
        ...
        if (t.type === TT.Keyword) {
          if (t.value === 'template') {
            if (peek == undefined || (peek.type !== TT.Symbol && peek.value !== '{'))
              throw new ftHTMLInvalidTypeError(peek, 'an opening and closing braces for \'template\' keyword');
    ...
    parseKeyword(keyword, index) {
      ...
      if (type === undefined || type !== TT.String) throw new ftHTMLInvalidTypeError(this.peek(index), 'string values only')
    ...
    parseTemplate(start, end) {
      ...
      for (let i = start + 2; i < end -1; i++) {
        ...
        if (t.type === TT.Keyword) {
          if (t.value !== 'import')
            throw new ftHTMLInvalidKeywordError(t);
        }
        else {
          if (t.type !== TT.Word)
        ...
        if (peek === undefined || peek.type !== TT.String)
          throw new ftHTMLInvalidTypeError(peek, 'a string value for template properties');
      ...
      if (!this.vars.import.import)
        throw new ftHTMLParserError(`templates require a valid import statement`, this.tokens[start]);

      this.updateStackForCaller("template", this.tokens[start]);
      return (new Parser({import: this.vars.import}).renderFile(path.resolve(this.vars._$.__dir, this.parseString(this.vars.import.import))));
    }
