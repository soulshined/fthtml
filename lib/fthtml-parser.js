(function () {
  ...
  const ftHTMLParser = {};
  const { ftHTMLError, ftHTMLInvalidCharError, ftHTMLexerError, ftHTMLParserError, ftHTMLInvalidTypeError, ftHTMLInvalidParentElementError, ftHTMLIncompleteElementError, ftHTMLInvalidKeywordError, ftHTMLInvalidElementNameError, ftHTMLInvalidVariableNameError, ftHTMLVariableDoesntExistError, ftHTMLImportError } = require('../utils/exceptions/fthtml-exceptions');
  ...
  ftHTMLParser.Parser = (class Parser {
    ...
    renderFile(file) {
      if (file.startsWith('https:') || file.startsWith('http:')) {
        throw new ftHTMLError(new ftHTMLImportError(`Import files must be local, can not access '${file}'`));
      }
      ...
      try {
        ...
        if (fs.existsSync(file)) {
          ...
          throw new ftHTMLError(new ftHTMLImportError(`Can not find file '${file}' to import`));
        }
      }
      catch (err) { 
        if (err instanceof ftHTMLParserError || err instanceof ftHTMLexerError)
          console.log(`\n${(new ftHTMLError(err, this.vars._$.__stack)).toString()}`);
