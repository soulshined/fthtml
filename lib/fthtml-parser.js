(function () {
    ...
    ftHTMLParser.Parser = (class Parser {
        ...
        parse(tokens, isChildIteration = false) {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                if (t.type === TT.Keyword) {
                    if (t.value === 'template') {
                        if (peek === undefined || peek.type !== TT.Symbol || (peek.type === TT.Symbol && peek.value !== '{'))
                            throw new ftHTMLInvalidTypeError(peek === undefined ? t : peek, 'an opening and closing braces for \'template\' keyword', stackTrace);
                ...
                if (t.type === TT.ELang) {
                    if (peek === undefined || peek.type === undefined || (peek.type !== TT.Symbol && peek.value !== '{'))
                ...
                if (t.type === TT.Word) {
                    if (peek === undefined || [TT.Keyword, TT.Word, TT.ELang].includes(peek.type) || (peek.type === TT.Symbol && peek.value === '#')) {
            ...
            for (let i = start; i < end; i++) {
                ...
                if (t.type === TT.Symbol) {
                    ...
                    if (t.value === '#') {
                        ...
                        if (peek.type === TT.Variable)
                            throw new ftHTMLInvalidTypeError(peek, 'a selector or identifier', stackTrace);
        ...
        parseKeyword(keyword, index) {
            ...
            if (peek === undefined)
                throw new ftHTMLIncompleteElementError(this.tokens[index], "", 'a value', stackTrace);

            const { type, value } = peek;
            if (type !== TT.String) throw new ftHTMLInvalidTypeError(this.peek(index), 'string values only', stackTrace)
        ...
        findPragmaEnd(start) {
            ...
            for (let i = start; i < this.tokens.length; i++) {
                const t = this.tokens[i];
                const peek = this.peek(i);
                if (peek !== undefined && t.type === TT.Symbol && t.value === '#' && peek.type === TT.Keyword && peek.value === 'end') {
                    return i + 1;
                }
            }
