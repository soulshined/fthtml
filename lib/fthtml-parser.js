  ...
  ftHTMLParser.Parser = (class Parser {
          ...
          if (e.type === TT.Word) {
            if (this.peek(i).type === TT.Symbol && this.peek(i).value === '=') {
              if (this.peek(i + 1).type === TT.Word || this.peek(i + 1).type === TT.String || this.peek(i + 1).type === TT.Variable) {
                if (this.peek(i + 1).type === TT.String) attrs += ` ${e.value}="${this.parseString(this.peek(i + 1).value)}"`;
                else if (this.peek(i + 1).type === TT.Variable) attrs += ` ${e.value}="${this.parseVarToken(this.peek(i + 1))}"`;
    ...
    parseString(value) {
      ...
      while ((matches = regx.exec(str)) !== null) {
        const match = this.vars[matches[1]];
