...
ftHTMLexer.TokenStream = function (input) {
    ...
    function tokenize() {
        ...
        if (index_2.default.rules.isIdentifierChar(ch))
            ...
            input.next();
            return types_1.Token("Symbol", ch, pos);
        }
        else
    ...
    function readComments() {
        ...
        if (peek === '/') {
            ...
            const peek = input.peek();
            if (peek === '/')
                return types_1.Token("Comment", `/${readWhile(ch => ch != '\n')}`, pos);
            else if (peek === '*')
                return types_1.Token("Block Comment", `/*${readBlockComment()}`, pos);
            else
    ...
    function readIdentifier() {
        ...
        if (type === "Pragma")
            throw new index_1.ftHTMLexerError('Pragma not well-formed, required "#" prefix missing', pos);
        if (type === "ELang") {
            LM = 0;
    ...
    function readString() {
        ...
        return types_1.Token("String", readEscaped(delimiter), pos, delimiter);
    }
    function readVariable() {
        ...
        if (identifier.length === 0)
            throw new index_1.ftHTMLInvalidCharError('@', pos);
        return types_1.Token("Variable", identifier, pos);
    }
    function readElang() {
        const pos = input.position();
        input.next();
            ...
            if (_lm != 1) {
                if (ch == '{')
                    ...
                    if (--openBraces === 0) {
                        ...
                        return types_1.Token("ElangB", buffer, input.position());
                    }
                }
    ...
    function readClassAttr() {
        ...
        if (index_2.default.rules.isWhitespace(input.peek()) || input.eof())
            ...
            if (identifier.length === 0)
                throw new index_1.ftHTMLInvalidCharError('@', pos);
            return types_1.Token("Attr_Class_Var", identifier, pos);
        }
        return types_1.Token("Attr_Class", readWhile((ch) => {
            if (index_2.default.rules.isIdentifierChar(ch))
                return true;
            else if (ch == '.' || ch == '#')
                throw new index_1.ftHTMLInvalidCharError(ch, input.position());
