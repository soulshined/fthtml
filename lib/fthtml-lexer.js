(function () {
  ...
  ftHTMLexer.Lexer = (class Lexer {
    constructor(stackTrace = []) {
      ...
      this.keywords = ['doctype', 'comment', 'import', 'end', 'template'];
      this.elangs = {
        js : {
          stringSyms : ["'",'"','`']
        },
          ...
          case ftHTMLexer.LexMode.Definition: {
            if (this.isValidChar(c)) {
              ...
              switch (c) {
                ...
                case '"':
                case "'":
                  if (buffer.length > 0) this.throwInvalidCharError(c, line, pos);
                ...
                default:
                  if (c === ' ' || c === '\n' || this.symbols.includes(c)) {
                    if (buffer.length > 0) {
                      ...
                      if (buffer.length === 1 && ['/', '*'].includes(buffer)) this.throwInvalidCharError(buffer, line, pos-1);
                    ...
                    if (!['\t', '\r'].includes(c)) this.throwInvalidCharError(c, line, pos);
                    continue;
                  }
              }
            }
    ...
    throwInvalidCharError(c, line, pos) {
      throw new ftHTMLInvalidCharError(c, line, pos, this.stackTrace);
    }
  });
